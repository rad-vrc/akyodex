...
│   │       ├── categories/          # GET/POST - Category tree, create/rename/merge/delete (one commit)
...
#### `GET /api/categories`
**List categories** (admin or owner). Reads `data/akyo-data-ja.csv`, `data/category-translations.json` and `src/lib/category-colors.json` from the current `main` commit.

**Response**:
```json
{
  "head": "<commit sha>",
  "categories": [{ "path": "動物/うま", "en": "Animal/Horse", "ko": "동물/말", "count": 12 }],
  "colors": { "動物": "#d44335" }
}
```

#### `POST /api/categories`
**Create / translate / rename / merge / delete a category**. `create` and `translate` are open to admins; `rename`, `merge` and `delete` are owner-only. Every call is one commit on `main` (Git Data API) that updates the JA CSV, the translation table and the colour table together; EN/KO CSV/JSON are regenerated by the Sync JSON Data workflow.

**Body**:
```json
{ "action": "create",    "path": "動物/ねこ", "en": "Cat", "ko": "고양이" }
{ "action": "translate", "path": "動物",      "en": "Animal", "ko": "동물" }
{ "action": "rename",    "from": "動物", "to": "生き物", "en": "Creature", "ko": "생물" }
