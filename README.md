...
- **Web Vitals**: Deferred client-side reporting via `src/components/web-vitals.tsx`
...
- **Testing**: Playwright + focused unit tests colocated with source files
...
├── next.config.ts                   # Next.js 16 / image / security headers
├── open-next.config.ts              # OpenNext for Cloudflare Pages
├── wrangler.toml                    # Pages, R2, KV bindings
...
├── src/
│   ├── app/
│   │   ├── layout.tsx               # Metadata, CSP nonce wiring, runtime features
│   │   ├── page.tsx                 # / -> /zukan redirect entry
│   │   ├── manifest.ts              # Web App Manifest
│   │   ├── offline/                 # Offline fallback page
│   │   ├── admin/                   # Admin UI
│   │   ├── zukan/                   # Gallery UI and state helpers
│   │   └── api/                     # Admin / data / VRChat / cache APIs
│   ├── components/
│   │   ├── runtime-features.tsx     # Deferred SW / Web Vitals / Dify bootstrap
│   │   ├── dify-chatbot.tsx         # Dify embed loader
│   │   ├── akyo-card.tsx            # Card UI
│   │   ├── filter-panel.tsx         # Filter UI
│   │   └── admin/                   # Admin components and tabs
│   ├── hooks/
│   │   ├── use-akyo-data.ts
│   │   └── use-language.ts
│   ├── lib/
...
│   │   ├── session.ts
│   │   ├── vrchat-utils.ts
│   │   ├── vrchat-world-info.ts
│   │   ├── world-registration.ts
│   │   ├── booth-url.ts
│   │   └── cloudflare-image-loader.ts
│   ├── types/
│   └── middleware.ts                # i18n + CSP
├── scripts/
│   ├── csv-to-json.ts
│   ├── fix-opennext-instrumentation.js
│   ├── prepare-cloudflare-pages.js
│   ├── verify-dify-csp-hash.js
│   ├── generate-social-images.js
│   ├── compress-social-images.js
│   └── push-and-check-pr-conflicts.js
├── tests/                           # Playwright fixtures / regression tests
└── data/
    ├── akyo-data-ja.csv
    ├── akyo-data-en.csv
    ├── akyo-data-ko.csv
...
npm run next:build:opennext # Next.js build + OpenNext instrumentation fix
npm run start            # Start production server (local)

# Quality
npm run lint             # Run ESLint
npm run knip             # Dead code analysis
npm run verify:dify-csp-hash # Verify middleware CSP hash matches current Dify embed
npm run hooks:install    # Configure local git hooks path
...
npm run generate-ko-data # Regenerate Korean dataset from JA source
npm run social:generate  # Generate social preview images
npm run social:compress  # Compress generated social images
npm run push:check-pr -- -u origin HEAD # Push and verify branch PR conflict state
```
...
| `NEXT_PUBLIC_SITE_URL` | Metadata / canonical URL fallback | `http://localhost:3000` |
...
| `NEXT_PUBLIC_USE_KV_DATA` | KV を優先するか（既定: `true`） | `false` |
| `NEXT_PUBLIC_USE_JSON_DATA` | JSON フォールバックを使うか（既定: `true`） | `true` |
| `NEXT_PUBLIC_ENABLE_CLOUDFLARE_IMAGES` | Cloudflare Images loader を有効化 | `true` |
| `NEXT_PUBLIC_CLOUDFLARE_IMAGES_ACCOUNT_HASH` | Cloudflare Images account hash | `xxxxx` |
...
| `NEXT_PUBLIC_SITE_URL` | Canonical / build-time site URL fallback | `https://akyodex.com` |
| `NEXT_PUBLIC_R2_BASE` | CDN base | `https://images.akyodex.com` |
| `NEXT_PUBLIC_DIFY_CHATBOT_TOKEN` | Udify token | *(optional)* |
| `NEXT_PUBLIC_USE_KV_DATA` | KV 読み出しを有効化 | `true` |
| `NEXT_PUBLIC_USE_JSON_DATA` | JSON フォールバックを有効化 | `true` |
| `NEXT_PUBLIC_ENABLE_CLOUDFLARE_IMAGES` | Cloudflare Images loader を有効化 | `true` |
| `NEXT_PUBLIC_CLOUDFLARE_IMAGES_ACCOUNT_HASH` | Cloudflare Images account hash | `xxxxx` |
...
| `NEXT_PUBLIC_SENTRY_DSN` | Browser-side Sentry DSN | *(optional)* |
| `SENTRY_ORG` / `SENTRY_PROJECT` / `SENTRY_AUTH_TOKEN` | Release upload / sourcemaps | *(optional)* |
...
Current runtime code reads `ADMIN_PASSWORD_OWNER`, `ADMIN_PASSWORD_ADMIN`, `SESSION_SECRET`, `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_R2_BASE`, and optional feature flags such as `NEXT_PUBLIC_USE_KV_DATA` / `NEXT_PUBLIC_USE_JSON_DATA`. `NEXT_PUBLIC_SITE_URL` is injected at build time by `next.config.ts`. The `DEFAULT_*_HASH` / `DEFAULT_JWT_SECRET` values are only referenced by the current workflow YAML.
...
- ✅ Offline support
- ✅ Deferred service worker registration after initial load
- ✅ Icon / image / API caching tuned per route type
- ✅ App-like experience
...
#### `GET /api/csv`
**Download raw CSV data by locale**

**Query Parameters**:
- `lang` (string, optional): `ja` / `en` / `ko` (`ja` default)

**Lookup order**:
1. Local `data/akyo-data-{lang}.csv`
2. R2 public URL (`/akyo-data/...`, then `/data/...`)
3. GitHub raw URL

**Response**: `text/csv`

#### `GET /api/manifest`
**Image manifest placeholder endpoint**

- Returns an empty JSON object for now.
- Kept for compatibility with the existing image fallback pipeline.

#### `POST /api/revalidate`
**On-demand ISR + tag revalidation**

**Headers**:
- `x-revalidate-secret`: required
...
When `updateKV` is `true`, the endpoint also refreshes the KV cache from JSON data.

#### `GET|POST /api/kv-migrate`
**KV migration and inspection endpoint**

- Protected by the same `x-revalidate-secret` header.
- Used to migrate JSON-backed data into KV and inspect migration status.
...
No license file is currently committed in this repository. Add `LICENSE` before claiming a distribution license in the README.
