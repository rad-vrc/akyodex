 ...
 */
async function setup(allAttributes: string[]) {
  ...
  for (const [key, value] of Object.entries({
    ...
    fetch: async (url: string, init?: RequestInit) => {
      ...
      // サーバーが返すのは実際に作った階層だけ。既にある親はここに出てこない
...
test('AttributeModal: 子を選ぶと親も選ばれ、親を外すと配下も外れる', async () => {
  const { applied, click, teardown } = await setup(['動物', '色', '色/紫色系', '色/紫色系/薄紫']);
  try {
...
test('AttributeModal: 作った直後は、既にある親も一緒に選ばれている', async () => {
  const { applied, button, click, flush, type, teardown } = await setup(['色']);
  try {
    // 親が既にある場合。サーバーは親を作らないので createdPaths にも出てこない
    ...
    // 親も子も新しい場合。まとめて作られた親も選ばれる
    await click('新しいカテゴリを作成');
    await type('attributeNewInput', '植物/木');
    await act(async () => button('追加する').click());
    await flush();
    await click('選択を決定');
    assert.deepEqual(applied.at(-1), ['色', '色/紫色系', '植物', '植物/木']);
  } finally {
    await teardown();
  }
});
