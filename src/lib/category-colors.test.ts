 ...
 * - 芸術は「芸術・アート」から階層化しても青灰を維持（2026-09-12 に赤へ。下記参照）
 * - 作風・スタイルは廃止した「電子」の落ち着いた赤を引き継ぐ
 * - グッズはフォールバックの緑ではなくシアン（2026-09-12 に赤へ。下記参照）
 * - 2026-09-12 に枠ごとの意味を決めて 22 カテゴリを移した。赤橙=有機的（動物・食べ物・体）、
 *   赤=人と社会・感情（Booth・パロディ・芸術・願い事・お大事に）、緑=自然と能力、
 *   青=物・場所・装い・存在と称号（家電・機械・道具・架空の存在・頂に立つ者）、
 *   シアン=対応機種のようなメタ情報（チョコミント類・レア）、
 *   青灰=感情の乗らない科学・抽象属性（器官・液体・エネルギー・状態・色・次元・始祖）
