...
- **`unjudged` が付いている** — 直近の取得が上の状態だったので、前回までの値を残して
  ある記録。値はあるが最新の判定ではない。

これらを「その機種に対応していない」と読み替えると、確認できていないのに既存の
Quest / iOS タグを消してしまう。**「PC の実ビルドを確認できた」ことと「実ビルドを取得
できなかった」ことは別に扱う。**

### `unjudged` が要る理由

値を守るだけでは足りない。古い成功キャッシュを黙って残すと、付与スクリプトはそれを
最新の判定として使ってしまう。

| 段階 | 内容 |
|---|---|
| 古いキャッシュ | PC のみ |
| 現在の CSV | あとから管理画面で Quest 対応を手入力（PC ＋ Quest） |
| `--refresh` の応答 | 開始前検査は成功、その後のワールド取得は 200 ＋ 空 |
| 値だけ残すと | 付与で **CSV の Quest タグが消える**。しかも次回は成功済みとして飛ばされる |

そこで、前回の値は保持しつつ `unjudged` に理由を書き残す。付与側はその行に触れず、
再開側は取り直しの対象にする。**取り直しに成功すると記録ごと差し替わり、`unjudged` は
自動的に消える。**

根拠のある記録を根拠のない結果で上書きしないのも同じ考え方で、`source: manual`
（オーナー申告）は API の失敗で無効化しない。

この経路は `scripts/vrchat-platform-categories.test.js` の通しテストで固定してある。
関数単位だけだと段と段のつなぎ目が抜けるので、取得 → 付与 → 再開を実際につないである。
